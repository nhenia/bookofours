...
export function getLunarStatus(date = new Date(), coords = { lat: 40.7128, lng: -74.0060 }) {
  ...
  // Logic for Void of Course (Simplified: Moon in the last 2 degrees of a sign)
  // A sign is 30 degrees. 2 degrees is ~6.6% of a sign.
  const progressInSign = (signProgress * 12) % 1;
  const isVoidOfCourse = progressInSign > 0.933;

  // Logic for Egyptian Days (April 10 and April 20)
  const month = date.getUTCMonth(); // 0-indexed, April is 3
  const day = date.getUTCDate();
  const isEgyptianDay = month === 3 && (day === 10 || day === 20);

  // Logic for Lunar Zenith (During the Office of Sext)
  const isZenith = currentOffice === "Sext";
